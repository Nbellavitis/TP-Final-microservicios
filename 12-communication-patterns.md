...
| API Gateway | Identity API / auth cache | Sync request/response or signed token introspection for `ValidateSessionForAction` | Room/Tournament commands depend on active session truth | If Identity is temporarily unavailable, gateway may use a very short-lived signed session cache; high-risk commands can fail closed. |
...
| Room Engine Pod | RNG/Deck Service | Internal sync RPC by seed/draw cursor | Supports server-authoritative seeded deck without exposing hidden state | If RPC fails before commit, command fails/retries with same `actionId`. Draw result is not broadcast unless the room transaction commits. |
...
## 4. Saga and process manager boundaries
